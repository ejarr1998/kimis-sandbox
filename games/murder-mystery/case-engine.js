...
const CaseEngine = (() => {
  ...
  const SETTINGS = [
    ...
    "a 24-hour diner off the interstate",
    "a tech startup's office on launch night",
    "the VIP backstage of a summer music festival",
    "a late-night drive-thru on the edge of town",
    "a co-working space during a weekend hackathon",
    "a cruise ship's final night at sea",
    "a true-crime podcast's live taping"
  ];
  ...
  const GEN_PROMPT = (setting, opts = {}) => {
    ...
    // quietly write a 1950s plot in a 2020s wrapper.
    const eraBlock = era ? `
THE ERA FOR THIS CASE IS FIXED BY THE HOST: ${era}. Everything — technology, forensics, culture, slang, fashion, and the artStyle — must be authentic to ${era}.${/present|modern|20[12]0s|future/i.test(era) ? " Modern life applies in full: smartphones, texting, social media, CCTV and DNA forensics all exist, and both the murder and its investigation must make sense in that world instead of pretending those tools away." : ""} If the chosen setting implies a different period, the era wins — re-stage that kind of place in ${era}.` : "";

    // Pre-cast suspects: host-supplied name+role pairs the writer MUST use.
    const castBlock = cast.length ? `
THE HOST HAS PRE-CAST ${cast.length} OF THE 6 SUSPECTS — they MUST appear with EXACTLY these names and roles:
...
Rules: use fixes[] for name/consistency/portrait/wording repairs (max 6), patches[] for solvability and difficulty gaps (max 3). Set solvable=false AND consistent=false only for fundamentally broken cases with no repair path.`;
  ...
  // options.era: optional decade/period the case MUST be set in.
  // options.cast: optional [{name, role}] suspects the writer MUST include.
  async function generateCase(onStatus, customSetting, options = {}) {
    ...
    onStatus?.(`🖋 Claude is writing the case… (${setting}${era ? ", " + era : ""})`);
    ...
    for (let attempt = 1; attempt <= 2; attempt++) {
      try {
        ...
        // The writer sometimes "improves" host-supplied names — hard-fail the
        // draft if any pre-cast suspect went missing, so the retry fixes it.
        for (const c of cast) {
          const found = (caseFile.suspects || []).some(s =>
            (s.name || "").toLowerCase().includes(c.name.toLowerCase()) ||
            c.name.toLowerCase().includes((s.name || "").toLowerCase()));
          if (!found) throw new Error(`pre-cast suspect "${c.name}" was dropped by the writer`);
        }
